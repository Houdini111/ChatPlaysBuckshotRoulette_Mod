...
func create_overwrite_asset(file_path: String) -> String:
	...
	mod_tool_store.editor_plugin.get_editor_interface().get_resource_filesystem().scan()
...
"""
	...
	mod_tool_store.editor_plugin.get_editor_interface().edit_script(overwrites_script)
...
func handle_override_creation(metadata: Dictionary) -> void:
	...
	current_script = mod_tool_store.editor_plugin.get_editor_interface().get_script_editor().get_current_script()
