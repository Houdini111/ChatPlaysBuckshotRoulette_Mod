...
func inject(injection_path: String, is_embedded := false) -> void:
	...
	ModLoaderSetupLog.debug("file_name extension: %s" % injection_path.get_extension(), LOG_NAME)
	arguments.push_back(
		(
			"--output=%s"
...
#					% [file_name[injection_path.get_extension()], injection_path.get_extension()]
					% [file_name["exe"], injection_path.get_extension()]
				)
			)
		)
	)
...
#	ModLoaderSetupLog.debug("Injection started: %s %s" % [path.gdre, arguments], LOG_NAME)
	ModLoaderSetupLog.debug_json_print("Injection started: %s Args:" % path.gdre, arguments, LOG_NAME)
	var output := []
	var _exit_code_inject := OS.execute(path.gdre, arguments, output)
	ModLoaderSetupLog.debug("Injection exit code: %s" % _exit_code_inject, LOG_NAME)
