...
static func get_dir_paths_in_dir(src_dir_path: String) -> Array:
	...
	if dir == null:
		...
		ModLoaderLog.error("Encountered an error %s (%s) when attempting to open a directory, with the path: %s" % [error_code, error_string(error_code), src_dir_path], LOG_NAME)
		return dir_paths
...
static func get_mod_paths_from_all_sources() -> Array[String]:
	ModLoaderLog.info("Searching for mod file paths", LOG_NAME)
	var mod_paths: Array[String] = []

	ModLoaderLog.debug("Getting mod dirs", LOG_NAME)
	var mod_dirs := get_dir_paths_in_dir(get_local_folder_dir("mods-unpacked"))

	ModLoaderLog.debug("Checking ModLoaderStore features for unpacked mods", LOG_NAME)
	...
	ModLoaderLog.debug("Checking ModLoaderStore features for local packed mods", LOG_NAME)
	...
	ModLoaderLog.debug("Checking ModLoaderStore features for steam workshop mods", LOG_NAME)
	if ModLoaderStore.ml_options.load_from_steam_workshop:
		mod_paths.append_array(_ModLoaderSteam.find_steam_workshop_zips())

	ModLoaderLog.debug_json_print("Found the following mod paths", mod_paths, LOG_NAME)
	return mod_paths
